...
{
    public MatLib materialLibrary;
    public static List<string> myList;
    [ListToPopup(typeof(MatConfig), "myList")]
    public string Popup;

    [ContextMenu("Create Number List")]
    public void CreateNumberList()
    {
        myList = materialLibrary.materialNames;
    }
...
#if UNITY_EDITOR
[CustomEditor(typeof(MatConfig))]
class MatConfigEditor : Editor
{
    public override void OnInspectorGUI()
    {
        var matConfig = (MatConfig)target;

        if (matConfig == null) return;

        Undo.RecordObject(matConfig, "Change matConfig");
    }
}
#endif
